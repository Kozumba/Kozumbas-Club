...
include "config.php";
//for action count
$stmt= $db->prepare("SELECT `category` FROM `usergames` WHERE category = 'action'");
$stmt->execute();

$totalaction = $stmt->rowcount();
//for adventure count
$stmt= $db->prepare("SELECT `category` FROM `usergames` WHERE category = 'adventure'");
$stmt->execute();

$totaladventure = $stmt->rowcount();

//for puzzle count
$stmt= $db->prepare("SELECT `category` FROM `usergames` WHERE category = 'puzzle'");
$stmt->execute();

$totalpuzzle = $stmt->rowcount();

//for racing count
$stmt= $db->prepare("SELECT `category` FROM `usergames` WHERE category = 'racing'");
$stmt->execute();

$totalracing = $stmt->rowcount();

//for point&click count
$stmt= $db->prepare("SELECT `category` FROM `usergames` WHERE category = 'pointandclick'");
$stmt->execute();

$totalpointandclick = $stmt->rowcount();

//for sports count
$stmt= $db->prepare("SELECT `category` FROM `usergames` WHERE category = 'sports'");
$stmt->execute();

$totalsports = $stmt->rowcount();

//for strategy count
$stmt= $db->prepare("SELECT `category` FROM `usergames` WHERE category = 'strategy'");
$stmt->execute();

$totalstrategy = $stmt->rowcount();

//for RPG count
$stmt= $db->prepare("SELECT `category` FROM `usergames` WHERE category = 'rpg'");
$stmt->execute();
...
<?php
include "config.php";
    $stmt= $db->prepare("SELECT * FROM usergames");
    $stmt->execute();
    $user_games = $stmt->rowcount();
if ($user_games === 0 ){
       echo "<br><br><h4 class='text-center text-white'>There are currently no user games added, feel free to add your own games through profile settings. <b>Only registered users can add the games!</b> </h4>";
   }
...
$stmt= $db->prepare("SELECT * FROM usergames WHERE category = '".$_GET['category']."' limit $sakt_no,$skaits_pa_lapu");
$stmt->execute();
}
elseif(isset($_GET['searchbtn'])) {
    ...
    $stmt= $db->prepare("SELECT * FROM usergames WHERE title LIKE '%".$_GET['search']."%' limit $sakt_no,$skaits_pa_lapu");
    $stmt->execute(); 
...
else
{
         ...
         $stmt= $db->prepare("SELECT * FROM usergames limit $sakt_no,$skaits_pa_lapu");
         $stmt->execute();
}
?>
         <?php
...
//izpilda ciklu, kas izveido navigācijas/lapas pogas ja notiek meklēšana
    $stmt=$db->prepare("select * from usergames WHERE title LIKE '%".$_GET['search']."%'");
    $stmt->execute();
    $visas_speles=$stmt->rowCount();
    if($visas_speles <= 0){
        echo '';
...
echo "</div><div class='text-center mx-auto m-5'>";
if (($_GET['page'] ?? 1) != 1) {
echo "<a class='btn btn-dark' href='?downloadablegames&search=",$_GET['search'],"&searchbtn=&page=",($lapa-1),"'>< </a>";
}
    for($i=1;$i<=$visas_lapas;$i++)
    {
        echo "<a class='btn btn-dark' href='?downloadablegames&search=",$_GET['search'],"&searchbtn=&page=".$i."'>".$i."</a> ";
    }
if ($lapa != $visas_lapas){
 echo "<a class='btn btn-dark' href='?downloadablegames&search=",$_GET['search'],"&searchbtn=&page=",($lapa+1),"'> ></a>";
}
echo "</div>";
  ...
  $stmt=$db->prepare("select * from usergames WHERE category = '".$_GET['category']."'");
  $stmt->execute();
  $visas_speles=$stmt->rowCount();
  if($visas_speles <= 0){
    echo '';
...
if (($_GET['page'] ?? 1) != 1) {
echo "<a class='btn btn-dark' href='?downloadablegames&category=",$_GET['category'],"&page=",($lapa-1),"'>< </a>";
}
  for($i=1;$i<=$visas_lapas;$i++)
  {
      echo "<a class='btn btn-dark' href='?downloadablegames&category=",$_GET['category'],"&page=".$i."'>".$i."</a> ";
  }
if ($lapa != $visas_lapas){
echo "<a class='btn btn-dark' href='?downloadablegames&category=",$_GET['category'],"&page=",($lapa+1),"'> ></a>";
}
